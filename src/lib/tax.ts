...
export interface TaxRule {
  id: string;
  country: string;
  fund_id: string | null; // fund_config id when the rule is fund-specific, else null
  asset_type: string; // e.g. MUTUAL_FUND_OPEN_ENDED
  tax_type: TaxType;
  transaction_type: string; // e.g. REDEMPTION, DISTRIBUTION
  taxpayer_type: string | null; // e.g. INDIVIDUAL_RESIDENT
  rate_pct: number | null; // null until officially verified
  rate_type: "PERCENT" | "FLAT";
  holding_period_days: number | null;
  holding_period_operator: "LT" | "LE" | "GE" | "GT" | null; // applies when held <op> this many days
...
export function getExitLoadSchedule(fundName: string): ExitLoadSchedule | null {
  ...
  if (!meta) return null;
  const schedule = EXIT_LOAD_SCHEDULES[meta.name];
  if (!schedule) return null;
  return { ...schedule, fundName: meta.name, status: "VERIFIED" };
}
